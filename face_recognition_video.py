...
# Загрузка модели определения лиц из keras
...
# СОздание базы с лицами
base = {}
for dirname in ['julia', 'natalia']:# вставить имена людей, которых надо узнать
    base[dirname] = []
    for file in os.listdir('samples/people' + dirname):
        if file.endswith('.jpg'):
            #Загрузка картинки с лицом
            image = cv2.imread('samples/people' + dirname + '/' + file)
            # Заменяем цветокоррецкию на RGN
            frame = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            # Получение размеров изображения
            image_size = numpy.asarray(image.shape)[0:2]
            #Получение списка лиц с координатами и щзначением уверенности
            ...
            if faces_boxes:
                ...
                #Выравнивание лиц
                d = h - w# Разница между выостой и шириной
                w += d#КВАДРАТИЗАЦИЯ
                ...
                #Сохранение суммы евклидова пространства (в инете сказали, что так надо)
                base[dirname].append(get_distance(embedder, image))
# Инициализация счётчика кадров и лиц соответственно
frame_id = 0
face_n = 0
    ...
    if success:
        ...
        if faces_boxes:
            ...
            for face_box in faces_boxes:
                ...
                w += d  # Делаем изображение квадратным
                x = numpy.maximum(x - round(d/2), 0)
                x1 = numpy.maximum(x, 0)
                y1 = numpy.maximum(y, 0)
                x2 = numpy.minimum(x + w, image_size[1])
                ...
                if face_box['confidence'] > 0.99:  # 0.99 - уверенность сети в процентах что это лицо
                    ...
                    if identity:
                        ...
                        cv2.imwrite('samples/recognition_video/output/faces/recognized/' + str(median) + '.' + str(face_n)
                                    + '.jpg', face_image)
        ...
        cv2.imwrite('samples/recognition_video/output/frames/' + str(frame_id) + '.jpg', image_detected)
